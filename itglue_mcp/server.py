"""IT Glue MCP server (FastMCP).

Run via:
    python -m itglue_mcp                        # stdio MCP server
    python -m itglue_mcp config init --config ./itglue.json
    python -m itglue_mcp ping --config ./itglue.json

Environment / config precedence is handled by :mod:`.config`. Credentials come
from ITGLUE_* env vars or a JSON config file written by ``config init``.

By default the network (http/sse/streamable-http) transports are gated behind a
bearer API key (``ITGLUE_MCP_AUTH_TOKEN``); every endpoint except the
...
def build_server(config=None):
    """Build a configured FastMCP app. ``config`` may be an ITGlueConfig or
    ...
    mcp = FastMCP(
        ...
        version=__version__,
        instructions=(
            "Tools to search and read IT Glue internal documentation for "
            "helpdesk work. Typical flow: search_documents (by name) in the "
            "configured organization, then get_document to read the full "
            "content and get_document_images to inspect embedded screenshots or "
            "diagrams. Use the returned document url for internal notes; use "
            "search_document_contents to search inside document text. "
            "list_organizations/get_organization resolve organization ids."
        ),
    )
    ...
    @mcp.tool()
    def itglue_config() -> dict:
        """Return a redacted description of the IT Glue environment this server
        is connected to (base URL, subdomain, default organization, ssl/timeout).
        Never includes the API key or MCP auth token."""
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="itglue-mcp",
        description="IT Glue MCP server (FastMCP).",
    )
    ...
    p_cfg = sub.add_parser("config", help="Configure IT Glue credentials")
    ...
    try:
        clear_client(load_config(config_file=args.config))
    except Exception:
        pass
    return 0
